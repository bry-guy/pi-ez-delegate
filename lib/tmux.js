...
export async function launchInTmux({ target, cwd, workerName, command, env, originPaneId }) {
  if (!env?.TMUX) {
  ...
  if (target === "pane") {
    ...
    const { stdout } = await runTmuxCommand(
      ["split-window", "-d", "-P", "-F", "#{pane_id}\t#{window_id}\t#{session_id}", "-t", originPane.paneId, "-c", cwd, command],
      { env },
    );
