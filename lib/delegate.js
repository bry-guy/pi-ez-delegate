...
export function formatDelegateHelp(topic) {
  if (topic === "start") {
    return [
      `Usage: /${DELEGATE_COMMAND} start [--target pane|window|session] [--name worker-name] [--cwd path] [--model pattern] [--no-worktree] [--no-print] <task>`,
      "",
  ...
  if (topic === "open") {
    return [
      `Usage: /${DELEGATE_COMMAND} open <name-or-id> [--target pane|window|session] [--model pattern]`,
      "",
