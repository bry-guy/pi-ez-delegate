...
export const DELEGATE_TARGETS = ["pane", "window"];
...
function parseOpenSubcommand(tokens) {
  const request = { nameOrId: "", target: undefined, model: undefined };
  const errors = [];
...
export function validateDelegateRequest(input) {
  ...
  if (!normalized.task) {
    throw new Error(`Usage: /${DELEGATE_COMMAND} [--target pane|window] [--name worker-name] [--cwd path] [--no-worktree] [--no-print] <task>`);
  }
...
export function formatDelegateHelp(topic) {
  if (topic === "start") {
    return [
      `Usage: /${DELEGATE_COMMAND} start [--target pane|window] [--name worker-name] [--cwd path] [--model pattern] [--no-worktree] [--no-print] <task>`,
      "",
      "Fork the current session into a new worker.",
      'Target "window" uses a shared tmux delegates window and places workers there as panes.',
    ].join("\n");
  }
  ...
  if (topic === "open") {
    return [
      `Usage: /${DELEGATE_COMMAND} open <name-or-id> [--target pane|window] [--model pattern]`,
      "",
...
async function resolveDelegateOrigin(branchEntries, env, options = {}) {
  ...
  if (currentOrigin) {
    return {
      ...
      source: storedOriginPaneId ? "rebound" : "env",
      reboundFromPaneId: storedOriginPaneId && storedOriginPaneId !== currentOrigin.paneId ? storedOriginPaneId : undefined,
    };
  }

  if (options.optional) return undefined;
  if (storedOriginPaneId) {
    throw new Error(`Stored origin pane no longer exists: ${storedOriginPaneId}. No current tmux pane is available to rebind the launch origin.`);
  }
...
function hasDelegateTaskToolCall(entry) {
  ...
  return entry.message.content.some(
    (part) => part?.type === "toolCall" && part.name === "delegate_task",
  );
}
  ...
  for (const entry of entries) {
    if (hasDelegateTaskToolCall(entry)) {
      for (const part of entry.message.content) {
        if (part?.type === "toolCall" && part.name === "delegate_task" && part.id) {
...
export function buildTmuxCommand({ piCommand, sessionFile, prompt, printMode, model }) {
...
export async function delegateTask(request, runtime) {
  ...
  try {
    ...
    const launch = await withPaneLaunchQueue(doLaunch);
