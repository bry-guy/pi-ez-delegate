...
function formatAttachHint(result) {
...
async function launchInTmux({ target, cwd, workerName, sessionFile, prompt, piCommand, env }) {
  if (!env?.TMUX) {
    throw new Error("pi-ez-delegate currently requires running inside tmux.");
  }

  const command = buildTmuxCommand({ piCommand, sessionFile, prompt });
  let args;

  if (target === "pane") {
    args = ["split-window", "-d", "-P", "-F", "#{pane_id}", "-c", cwd, command];
  } else if (target === "window") {
    args = ["new-window", "-d", "-P", "-F", "#{window_id}", "-n", workerName, "-c", cwd, command];
  } else {
    args = ["new-session", "-d", "-P", "-F", "#{session_id}", "-s", workerName, "-c", cwd, command];
  }

  const { stdout } = await runCommand("tmux", args, { env });
  const targetId = stdout.trim();
  return {
    adapter: "tmux",
    mode: target,
    targetId,
    sessionName: target === "session" ? workerName : undefined,
    attachHint: formatAttachHint({ mode: target, targetId, sessionName: target === "session" ? workerName : undefined }),
  };
}
...
export async function delegateTask(request, runtime) {
  ...
  const worker = deriveWorkerName(normalized.task, normalized.name);
