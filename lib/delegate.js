...
export const DELEGATE_SUBCOMMANDS = ["start", "list", "attach", "open", "clean", "help"];
...
export function formatDelegateHelp(topic) {
  if (topic === "start") {
    return [
      `Usage: /${DELEGATE_COMMAND} start [--target pane|window|session] [--name worker-name] [--cwd path] [--no-worktree] <task>`,
      "",
...
function buildTmuxCommand({ piCommand, sessionFile, prompt }) {
  const parts = [shellQuote(piCommand), "--session", shellQuote(sessionFile)];
  if (prompt) parts.push(shellQuote(prompt));
  return parts.join(" ");
}

export function buildDelegatedPrompt({ task, workerName, parentCwd, requestedCwd, effectiveCwd, worktree, automerge }) {
  const lines = [
  ...
  if (automerge && worktree?.created && worktree.mainCheckoutPath && worktree.taskBranch) {
    const mcp = worktree.mainCheckoutPath;
    const tb = worktree.taskBranch;
    const wp = worktree.worktreePath;
    lines.push(
      "",
      "When your task is complete and all changes are committed:",
      `1. Merge your branch back:   git -C ${shellQuoteForPrompt(mcp)} merge ${tb}`,
      `2. Remove this worktree:     git -C ${shellQuoteForPrompt(mcp)} worktree remove ${shellQuoteForPrompt(wp)}`,
      `3. Delete the task branch:   git -C ${shellQuoteForPrompt(mcp)} branch -d ${tb}`,
    );
  }
...
export async function delegateTask(request, runtime) {
  ...
  try {
    ...
    // Plan pane layout when target is pane
    let layoutPlan;
