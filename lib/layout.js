export function planTmuxPaneLaunch(input = {}) {
  ...
  if (splitPreference === "vertical") {
    ...
    if (windowWidth < 2 * minCols) {
      throw new Error(
        `Window too narrow for vertical split (need ${2 * minCols} columns). Use --target window.`,
      );
    }
  ...
  if (splitPreference === "horizontal") {
    ...
    if (enforceRowLimit && target.height < 2 * minRows) {
      throw new Error(
        `Pane too short for horizontal split (need ${2 * minRows} rows). Use --target window.`
      );
    }
  ...
  if (railPaneIds.length === 0) {
    ...
    throw new Error(
      `Not enough space for a pane split (need ${2 * minCols} columns or ${2 * minRows} rows). Use --target window.`
    );
  }
  ...
  if (enforceRowLimit && tallest.height < 2 * minRows) {
    throw new Error(
      `Rail panes too short for another split (need ${2 * minRows} rows). Use --target window or clean old delegates.`
    );
  }
