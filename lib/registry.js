...
function normalizeWorkerRecord(input) {
  ...
  return {
    ...
    tmuxSessionName: normalizeOptionalString(input.tmuxSessionName),
    model: normalizeOptionalString(input.model),
    effort: normalizeOptionalString(input.effort),
    resultFile: normalizePath(input.resultFile),
    worktreeMode: Boolean(input.worktreeMode),
  };
}
...
export function createWorkerRegistryRecord(result) {
  ...
  const sessionId = result?.launch?.sessionName || result?.launch?.targetId || result?.launch?.sessionId;
  ...
  return normalizeWorkerRecord({
    ...
    sessionId,
    tmuxSessionName: result?.launch?.sessionName,
    model: result?.request?.model,
    effort: result?.request?.effort,
    resultFile: result?.resultFile,
    worktreeMode: Boolean(result?.worktree?.created),
  });
}
