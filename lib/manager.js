...
export function getWorkerLivenessProbePlan(record) {
  const targetId = record?.tmuxSessionName || record?.sessionId || record?.targetId;
  return targetId ? [{ mode: "session", targetId }] : [];
}

export function getWorkerTmuxTarget(record, _options = {}) {
  const targetId = record?.tmuxSessionName || record?.sessionId || record?.targetId;
  return {
    targetMode: "session",
    targetId,
    sessionName: targetId || record?.slug,
  };
}

export async function readDelegateResultFile(resultFile) {
  if (!resultFile || !(await pathExists(resultFile))) return undefined;
  const raw = await readFile(resultFile, "utf8");
  try {
    return JSON.parse(raw);
  } catch (error) {
    throw new Error(`Could not parse delegate result at ${resultFile}: ${error.message}`);
  }
}
...
export async function inspectWorker(record, options = {}) {
  ...
  const gitSummary = summarizeGitState(gitState);
  const resultExists = record.resultFile ? await pathExists(record.resultFile) : false;
...
export async function reopenWorker(record, options = {}) {
  ...
  const effort = options.effort || record.effort;
  ...
  const effortFlag = process.env.PI_EZ_DELEGATE_EFFORT_FLAG || "--effort";
  const effortArg = effort ? ` ${effortFlag} ${shellQuote(effort)}` : "";
  const command = `${shellQuote(piCommand)}${modelArg}${effortArg} --session ${shellQuote(record.childSessionFile)}`;
...
export async function cleanSafeWorkers(registryResult, inspectedWorkers) {
  ...
  for (const worker of safeWorkers) {
    ...
    try {
      ...
      if (r.taskBranch) actions.push(`kept branch ${r.taskBranch}`);
...
export function formatWorkerList(inspectedWorkers) {
  ...
  for (const [status, workers] of Object.entries(grouped)) {
    ...
    for (const w of workers) {
      ...
      const target = r.tmuxSessionName || r.targetId ? ` [session]` : "";
      const git = w.gitSummary && w.gitSummary !== "missing" ? ` (${w.gitSummary})` : "";
      const branch = r.taskBranch ? ` ${r.taskBranch}` : "";
      const result = r.resultFile ? ` result:${w.resultExists ? "yes" : "pending"}` : "";
      lines.push(`  ${name}${task}${target}${branch}${git}${result}`);
      if (status === WORKER_STATUS.DEAD_NEEDS_ATTENTION) {
        lines.push(`    → /delegate open ${r.slug || r.id} to resume`);
      }
    }
  }
...
export function formatCleanPreview(inspectedWorkers) {
  ...
  if (safe.length > 0) {
    ...
    for (const w of safe) {
      ...
      if (r.taskBranch) parts.push(`keeps branch ${r.taskBranch}`);
  ...
  if (safe.length > 0) {
    lines.push("\nRun /delegate clean --yes to proceed.");
  }
  return lines.join("\n");
}
...
export function formatCleanResult(result) {
  ...
  if (result.needsAttention.length > 0) {
    ...
    for (const w of result.needsAttention) {
      lines.push(`  ${w.record.name || w.record.id} — ${w.gitSummary}`);
      lines.push(`    → /delegate open ${w.record.slug || w.record.id} to resume`);
    }
  }
