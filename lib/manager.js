...
export function getWorkerTmuxTarget(record, options = {}) {
  const fallbackMode = options.defaultMode === "window" ? "window" : "pane";
  const targetMode = record?.targetMode === "window" ? "window" : fallbackMode;

  let targetId = targetMode === "window" ? record?.windowId : record?.paneId;

  if (!targetId && options.allowFallback !== false) {
    targetId = record?.paneId || record?.windowId;
  }
...
export async function reopenWorker(record, options = {}) {
  ...
  const requestedTarget = options.target || record.targetMode;
  const targetMode = requestedTarget === "pane" ? "pane" : "window";
  ...
  const command = `${shellQuote(piCommand)}${modelArg} --session ${shellQuote(record.childSessionFile)}`;
